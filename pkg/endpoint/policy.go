...
func (e *Endpoint) applyL4PolicyLocked(labelsMap *identityPkg.IdentityCache,
	...
	if oldL4Policy != nil {
		for _, filter := range oldL4Policy.Ingress {
			attemptedRemovedIngressMapEntries := e.removeOldFilter(labelsMap, &filter, policymap.Ingress)
			setMapOperationResult(removedPolicyMapEntries, attemptedRemovedIngressMapEntries)
		}

		for _, filter := range oldL4Policy.Egress {
			_ = e.removeOldFilter(labelsMap, &filter, policymap.Egress)
	...
	for _, filter := range newL4Policy.Ingress {
		secIDs, errs = e.applyNewFilter(labelsMap, &filter, policymap.Ingress)
		setMapOperationResult(addedPolicyMapEntries, secIDs)
		errors += errs
	}

	for _, filter := range newL4Policy.Egress {
		_, errs = e.applyNewFilter(labelsMap, &filter, policymap.Egress)
		// TODO update maps for egress. Not touching conntrack for now.
		errors += errs
	}
